...
test("preview server exposes the current UI and mock chat endpoints without Wrangler", async () => {
  ...
  try {
    ...
    assert.match(pageHtml, /topBarSurface/);
    assert.match(pageHtml, /modelCatalogSurface/);
    assert.match(pageHtml, /sessionSidebar/);
    assert.match(pageHtml, /chatColumnSurface/);
    assert.match(pageHtml, /chatComposerShell/);
    assert.match(pageHtml, /chatHistorySurface/);
